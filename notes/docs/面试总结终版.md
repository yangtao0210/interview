...
**4）rows:表示mysql估计要读取 & 检测的行数，越少越好<font color = yellow>（不是结果集里的行数）</font>**
...
### 7.2 InnoDB在RR级别可以解决幻读，为何还要Serializable隔离级别?
...
- **公平锁实现**
  ...
  3.state != 0:判断锁的所有者是不是当前线程，若是，则更新state状态，返回true;否则，将当前线程加入同步队列等待并返回false。
  ```
